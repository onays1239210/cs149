...
/**
 * This is the algorithm that checks a process for its Shortest Remaining Time(preemptive).
 *
 */
public class SRT
{
  ...
  /**
     * The run method runs the Shortest Remaining Time algorithm on new processes entered. 
     ...
     */
    private static boolean checkStarvedProcesses(Collection<Process> processes, ReadyToRunQueue readyQueue, 
            LinkedList<Process> waitQueue, float currentTime, 
            float targetT, boolean hasStarved)
    {
       ...
       /**
     * This method runs a process through the SRT algorithm timeslice 
    ...
    private static String runTSlice(ReadyToRunQueue readyQueue, LinkedList<Process> waitQueue, float currentTime)
    {
        ...
        for(Process proc: waitQueue)
        {
            ...
            //The incoming process will replace the current if it has less remaining runtime

                 if(proc.getArrivalTime() <= 0 && !readyQueue.isEmpty() && proc.getRunTime() < readyQueue.getCurrProcess().getRunTime()) 
